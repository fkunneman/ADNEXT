...
"""
Script to perform ensemble classification on a voting basis 
"""
parser = argparse.ArgumentParser(description = "Script to perform ensemble classification on a voting basis ")
...
parser.add_argument('-c', action = 'store', nargs='+', choices = ["svm","nb","tree"],
    help = "choose the classifiers to vote on")
parser.add_argument('-a', action = 'store', nargs='+', required = False,
    help = "give files with classification results of a non-sklearn classifier")
parser.add_argument('-v', action = 'store', default = "arbiter", choices = ["weighted","arbiter"],
    help = "specify the type of voting")
parser.add_argument('-t', action = 'store', default = "svm", choices = ["svm","ripper"],
    help = "specify the type of classification of voted instances")
...
parser.add_argument('-p', action = 'store', type=int,default=10, required = False,
...
def classify(tr,te):
    ...
    cl.index_features()
    cl.tenfold_train(args.f,args.c,args.p)
