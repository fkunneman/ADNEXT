...
for testevent in event_time_test.keys():
	print testevent
	outfile = open(outputdir + "median_" + testevent + ".txt","w")
	...
	for bucket in hours:
		#if passed:
		outfile.write(str(bucket) + " " + str(median) + "\n")
		#else: 
		#	try:
		#		tweets = len(tweet_event_timedif_test[bucket])
		#		if tweets > 0:
		#			num_tweets += tweets
		#			if num_tweets > min_threshold:
		#				passed = True
		#				outfile.write(str(bucket) + " " + str(median) + "\n")
		#	except KeyError:
		#		continue
