...
for date in dates:
    ...
    try:
        for id_ in ids:
            os.system("wget -w 0.1 -v --user=f.kunneman@let.ru.nl --password=crawl2013 -i http://portal.anp.nl/rss/indexer.do?action=article\&id=" + id_ + "\&format=xml" + " -o " + logs + date_out + " -O " + date_out + id_)
    except:
